...
parser = argparse.ArgumentParser(
    description='Load a trained image classifier through a checkpoint and predict the top 5 most likely classes')
...
parser.add_argument('--classes', action='store', type=int, default=1,
...
flower_model = load_flower_checkpoint('checkpoint_dir/flower_checkpoint.pth')
...
probs, classes = predict(args.filepath, flower_model,
                         args.gpu, topk=args.classes)
...
if args.cat_path == None:
    ...
    with open(args.cat_path, 'r') as f:
        ...
        class_to_name = sorted(cat_to_name.items())
    # 2. Here is a list of class-index pairs: they were loaded from the checkpoint
    # we revert it in order to get an index-to-class mapping
    class_to_idx = list(flower_model.class_to_idx.items())
    idx_to_class = list(
        {v: k for k, v in flower_model.class_to_idx.items()}.items())
    # 3. Here we use the two lists created in points 1. and 2. to build a dictionary which associates indices to actual flower names
    idx_to_flower = {}
    for i in range(0, len(class_to_name)):
        idx_to_flower[idx_to_class[i][0]] = class_to_name[i][1]

    print(probs)
    print(list(map(idx_to_flower.get, classes)))