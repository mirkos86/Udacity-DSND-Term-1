...
def load_flower_checkpoint(filepath):
    ...
    model.classifier = nn.Sequential(nn.Linear(checkpoint['input_units'], checkpoint['hidden_units']),
                                     ...
                                     nn.Linear(
                                         checkpoint['hidden_units'], checkpoint['hidden_units']),
                                     nn.ReLU(),
                                     nn.Dropout(checkpoint['dropout']),
                                     nn.Linear(
                                         checkpoint['hidden_units'], checkpoint['output_units']),
                                     nn.LogSoftmax(dim=1)
                                     )
...
def process_image(image):
    ...
    if im.size[0] <= im.size[1]:
        ...
        ima = im.crop((16, int(im.size[1]-224)/2,
                       240, int(im.size[1]-224)/2+224))
    else:
        im.thumbnail([256/im.size[1]*im.size[0], 256])
        ima = im.crop((int(im.size[0]-224)/2, 16,
                       int(im.size[0]-224)/2+224, 240))
